...
在 DOM 中，它可能不是标准 `h2`，而是短段落、独立 `strong` / `b` 节点，或“只有一个加粗节点的段落”。扩展采集时：

- 如果短文本块像标题，并且位于 Claude AI 回复上下文中，应作为 `AI` 消息候选提交；不能因为它少于 20 个字就过滤掉。
...
- 如果短文本明显是句子而不是标题，例如以 `。`、`！`、`？`、`;` 结尾，应继续按普通短文本过滤，避免把碎片化句子误当标题。
