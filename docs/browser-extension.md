...
- Claude Artifact / 打开的文档面板：如果页面 DOM 中存在明确 Artifact 标记，或用户已经打开了带预览、编辑器、代码/Markdown 内容区的文档面板，扩展会尽量把它当作 Markdown 文本文件提交；如果 Claude 只把打开文档作为普通文本块混入 snapshot，后端会根据“artifact 标题块 + 后续完整文档块”再兜底识别一次。
