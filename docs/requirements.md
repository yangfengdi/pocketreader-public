...
- ChatGPT 当前页面导入应默认使用按回合拆分，标题编号行为和 Claude 保持一致。
- 同一个 AI 对话以相同朗读范围再次导入时，后端应按 `source_url + platform + reader_mode + 回合序号` 跳过已经存在且非 error 的回合，只为新增回合创建音频。
- “问题和 AI 回复”与“只读 AI 回复”是两组独立音频。例如先生成 `[AI答 001]`、`[AI答 002]`，再改选包含提问的模式时，应创建 `[问&答 001]`、`[问&答 002]` 以及届时已有的新回合。
- 拆分条目标题前缀包含朗读范围和当前回合序号，不包含动态变化的总回合数；格式为 `[问&答 001]` 或 `[AI答 001]`，编号固定三位，确保播客客户端列表中便于识别并保持顺序。
