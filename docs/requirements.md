...
- 同一个 AI 对话再次导入时，后端应按 `source_url + platform + 回合序号` 跳过已经存在且非 error 的回合，只为新增回合创建音频。
- 拆分条目标题前缀只包含当前回合序号，不包含动态变化的总回合数；格式使用固定三位数字，例如 `[001]`、`[002]`、`[012]`，确保播客客户端按标题或文本排序时能保持对话顺序。
